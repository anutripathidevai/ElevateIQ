...
import type { LLDProblemMeta } from "../types";

/**
 * Placeholder shown for catalog entries that don't yet have authored content.
 * Renders the known metadata (so the page is still useful) and clearly marks the
 * entry as coming soon.
 */
export function ComingSoon({
  ...
  const isConcept = meta.kind === "concept";
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-dashed border-border bg-muted/20 p-8 text-center">
        ...
        <h2 className="mt-4 text-lg font-semibold">
          {isConcept
            ? "This concept lesson is in the works"
            : "This deep-dive is in the works"}
        </h2>
        <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
          {isConcept ? (
            <>
              We&apos;re authoring a full lesson for{" "}
              <span className="font-medium text-foreground">{meta.title}</span> —
              theory, diagrams, worked Java examples, best practices, a quiz, and
              a cheat sheet. In the meantime, explore the published lessons.
            </>
          ) : (
            <>
              We&apos;re authoring a full 20-section breakdown for{" "}
              <span className="font-medium text-foreground">{meta.title}</span> —
              requirements, UML class + sequence diagrams, a complete Java
              implementation, dry run, tradeoffs, and interview follow-ups. In
              the meantime, explore the published problems.
            </>
          )}
        </p>
        <Link
