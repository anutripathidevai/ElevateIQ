...
import { toggleBookmark, useIsBookmarked } from "./progress-store";
...
function tierAccent(tier: LLDProblemMeta["tier"]) {
  return LLD_TIERS.find((t) => t.id === tier)?.accent ?? "emerald";
}

/**
 * A single catalog card for the dashboard. Published entries link to their page
 * and expose bookmark + solved state; coming-soon entries render a disabled,
 * clearly-labelled placeholder. Purely data-driven, works for both problems and
 * concept lessons.
 */
export function ProblemCard({ meta }: { meta: LLDProblemMeta }) {
