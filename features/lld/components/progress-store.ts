...
/**
 * Client-side bookmark + recently-viewed tracking for LLD entries, backed by
 * localStorage (mirrors `lib/progress-store.ts` and the System Design store).
 * Works fully in guest mode — no database or sign-in required.
 */
import { useSyncExternalStore } from "react";

const BOOKMARK_KEY = "elevateiq:lld:bookmarks:v1";
const RECENT_KEY = "elevateiq:lld:recent:v1";
const EVENT = "elevateiq:lld:store";
